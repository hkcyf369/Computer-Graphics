...
Color ray_color(Ray4& ray, int depth)
{
    ...
    if (got_a_hit) {
        ...
        if (debugOn) {
            cout << " ray (" << ray << ") HITS at:\n";
            cout << "P=" << hit.p << "\n";
            cout << "material " << hit.obj->material.name << "\n";
            cout << "depth=" << depth << "\n";
            cout.flush();
        }
...
Ray4 get_ray(int xDCS, int yDCS) {
    Point4 start = Point4(0,0,10);
    Vector4 direction = Vector4(0,0,-1);

    return Ray4(start, direction);
}
...
// Return true/false if there is/isn't a refracted ray.
// YOU MUST IMPLEMENT THIS FUNCTION.
//////////////////////////////////////////////////////
...
void read_scene(const char *filename) {
    ...
    while (!toker.eof()) {
        ...
        // v    v    v    v    v
            ...
            // Get the material specs, and make an light.
            // Add it to the map of lights.
            ;
        }
        else if (keyword == string("sphere")) {
            // Get the sphere, name it, and add it to the scene_objects.
            ;;
        }
        else if (keyword == string("triangle")) {
            // Get the triangle, name it, and add it to the scene_objects.
            ;
